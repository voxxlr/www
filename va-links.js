...
/*
        let viewpoint =  this.dom.querySelector("ui-dropdown-select").getAttribute("value");
        if (viewpoint)
        {
            data.dataset.viewpoint = viewpoint;
        }
        ...
        this.viewpoint.addEventListener("change", (event)=>
        {
            if (this.key)
            {
                ...
                ui-dropdown-select { width: 100%; }

                    <ui-dropdown-select placeholder="Viewpoint..." deletable></ui-dropdown-select>

    updateViewpoints(list)
    {
        let select = this.dom.querySelector("ui-dropdown-select");
        select.clear();
        if (list)
        {
            list.forEach(viewpoint => 
            {
                select.add(viewpoint.id, viewpoint.name)
            });
        }
    }
    this.dom.querySelector("ui-dropdown-select").clear();
    await fetch(`https://doc.voxxlr.com/meta/viewpoints`, 
    {
        headers: new Headers({ 'Authorization': `Bearer ${dataset.token}` })
    }).then(async (response) =>
    {
        this.updateViewpoints(await response.json());
    });
    */
